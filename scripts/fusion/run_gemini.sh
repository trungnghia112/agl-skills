...
#
# Config (env):
#   AGY_MODEL            model name to PIN (empty by default → use agy's configured default, i.e. the
#                        terminal default, so fusion tracks CLI upgrades without hard-coding a version).
#                        Set e.g. AGY_MODEL="Gemini 3.1 Pro (High)" to pin a specific/stronger tier
#                        (see `agy models`) if your agy default is a lighter model (e.g. a Flash tier).
#   FUSION_AGY_NO_MODEL  set to 1 to force-omit --model even if AGY_MODEL is set.
...
# Empty default → do NOT pass --model → agy uses its configured (terminal) default. Pin via AGY_MODEL.
AGY_MODEL="${AGY_MODEL:-}"
...
case "$(uname -s 2>/dev/null)" in
  MINGW*|MSYS*|CYGWIN*)
    ...
    if have python && [ -f "$cap_py" ]; then
      # No --model: use agy's configured (terminal) default, and passing --model in print mode
      # can hang (see bug notes). Opt in to a pinned model via FUSION_AGY_FORCE_MODEL.
